...
func (u *UserOnline) GetUserLock(uid string) *sync.RWMutex {
	u.Lock.Lock()
	defer u.Lock.Unlock()
	if u.UserLocks == nil {
		u.UserLocks = make(map[string]*sync.RWMutex)
	}
	lock := u.UserLocks[uid]
	if lock == nil {
		lock = &sync.RWMutex{}
		u.UserLocks[uid] = lock
	}
	return lock
}
...
func main() {
	...
	http.HandleFunc("/getFamilyDayTask", GetFamilyDayTaskH)
	...
	http.HandleFunc("/familyDayTask.html", FamilyDayTaskH)
	...
	running := true
	{
		go runner.NamedRunnerWithSeconds("RunnerPullAnimal", runnerPull, &running, RunnerPullAnimal)
		...
		go runner.NamedRunnerWithHMS("RunnerProcExchangeLast", 23, 10, 0, &running, ProcExchangeLast)
		go runner.NamedRunnerWithHMS("RunnerProcExchangeLast", 23, 46, 0, &running, ProcExchangeLast)
	}
	go RunnerBeach()
...
func FamilyDayTaskH(w http.ResponseWriter, req *http.Request) {
	t, _ := template.ParseFiles("familyDayTask.html")
	t.Execute(w, nil)
}
...
func GetMailPrizeH(w http.ResponseWriter, req *http.Request) {
	...
	for _, uid := range userIDs {
		...
		xlog.Infof("[%v] 查看邮件奖励[title:%v titleNew:%v type:%v]", user.Name, title, titleNew, cakeID)

		mailids := getMailList(user.ServerURL, user.ZoneToken, titleNew, cakeID)

		xlog.Infof("[%v] 开始领取邮件奖励[title:%v][mailids:%v]", user.Name, title, len(mailids))
		var i = 0
		for _, mailid := range mailids {
			...
			if getMailAttachments(user.ServerURL, user.ZoneToken, mailid) {
				i++
				xlog.Infof("[%v] 领取邮件完毕[%v]", user.Name, mailid)
				readMail(user.ServerURL, user.ZoneToken, mailid)
				xlog.Infof("[%v] 删除邮件完毕[%v]", user.Name, mailid)
			}
		}

		if len(mailids) > 0 {
			ss += fmt.Sprintf("[%v] 总共:%v 已领取:%v|||", user.Name, len(mailids), i)
		}

		xlog.Infof("[%v] 领取邮件完毕", user.Name)
	...
	if familyId != "" && input != "0" {
		go func() {
			...
			for rows.Next() {
				...
				rows.Scan(&userID)
				user := GetUser(userID)
				cancelFamilyRob(user.ServerURL, user.ZoneToken)
				xlog.Infof("[%s]取消啦动物", user.Name)
			}
			SQL := fmt.Sprintf("select id, token, name, pull_rows, follow_uids from tokens where familyId = %s", familyId)
			pullAnimalBySql(SQL)
		}()
	}
...
func ExchangeBeachRewardH(w http.ResponseWriter, req *http.Request) {
	...
	// 吧index1转成int64
	index, _ := strconv.ParseInt(index1, 10, 64)
	if id == "" || id == "all" {
		SQL := "select id from tokens"
		rows, err := Pool.Query(SQL)
		if err != nil {
		...
		for _, userID := range userIDs {
			...
			go func(userID string) {
				...
				items := data.Map("beachItems")
				v176 := items.Int64("176")
				v177 := items.Int64("177")
				v178 := items.Int64("178")
				v179 := items.Int64("179")
				v180 := items.Int64("180")
				v181 := items.Int64("181")
				...
				if index == 4 || index == 3 || index == 2 {
					if all < min {
						for {
							flag := exchangeBeachReward(user.ServerURL, user.ZoneToken, index)
							if !flag {
								break
							}
							xlog.Infof("%s兑换第%d套海滩奖励", user.Name, index)
						}
						io.WriteString(w, fmt.Sprintf("[%s]SUCCESS|", user.Name))
					} else {
						io.WriteString(w, fmt.Sprintf("[%s]:%d-%d-%d-%d-%d-%d|", user.Name, v176, v177, v178, v179, v180, v181))
					}
				} else {
					for {
						flag := exchangeBeachReward(user.ServerURL, user.ZoneToken, index)
		...
		if index == 4 || index == 3 || index == 2 {
			if all < min {
				...
				io.WriteString(w, fmt.Sprintf("%d-%d-%d-%d-%d-%d", v176, v177, v178, v179, v180, v181))
			}
		} else {
			for {
				flag := exchangeBeachReward(user.ServerURL, user.ZoneToken, index)
...
func addFirewoodLogic(id string, quality float64) {
	...
	for rows.Next() {
		...
		if uid == "302691822" && id == "system" && quality == 0 {
			xlog.Infof("302691822不参与添加全部类型的柴火")
			continue
		}
		userIDs = append(userIDs, uid)
	}
...
func GetFamilyDayTaskH(w http.ResponseWriter, req *http.Request) {
	id := req.URL.Query().Get("id")
	if id == "" {
		io.WriteString(w, "FAIL")
		return
...
func GetFamilyEnergyH(w http.ResponseWriter, req *http.Request) {
	...
	for _, userID := range ids {
		...
		d = append(d, xmap.M{"user_id": userID, "key": user.Name + "(" + userID + ")", "value": fmt.Sprintf("每日摇能量:%d,可用能量:%d", dayDraw, energy)})
	}
	io.WriteString(w, converter.JSON(d))
}
...
func DrawH(w http.ResponseWriter, req *http.Request) {
	...
	for _, uid := range userIDs {
		...
		zoneToken, familyDayTask := getEnterInfo(uid, user.Name, user.ServerURL, user.Token, user.ZoneToken, "familyDayTask")
		UpdateUser(uid, user.ServerURL, zoneToken, user.Token)
		// zoneToken := getZoneToken(serverURL, token)
		i++
		go func(user *User) {
			time.Sleep(time.Millisecond * 2000 * time.Duration(i))
			xlog.Infof("---------------------------[%v]开始转盘---------------------------", user.Name)
			xlog.Infof("[%v]【摇一摇】获取免费20能量", user.Name)
			getFreeEnergy(user.ServerURL, zoneToken)
			xlog.Infof("[%v]【摇一摇】领取好友能量", user.Name)
			autoFriendEnergy(user.ServerURL, zoneToken)
			uid := user.Uid
			energy := draw(uid, user.Name, user.ServerURL, zoneToken, drawMulti, isTask)
			...
			if intAmountAll == 0 {
				xlog.Infof("[%v]转盘次数不足", user.Name)
			} else {
				followCompanion_1(user.ServerURL, zoneToken, 2)
				for i := 1; i <= intAmountAll; i++ {
					xx := draw(uid, user.Name, user.ServerURL, zoneToken, drawMulti, isTask)
			...
			taskIDs := getDayTasksInfo(user.ServerURL, zoneToken)
			xlog.Infof("[%v]领取日常任务奖励:%v", user.Name, taskIDs)
			for _, taskID := range taskIDs {
				// time.Sleep(time.Millisecond * 100)
				getDayTaskAward(user.ServerURL, zoneToken, taskID)
			}

			xlog.Infof("[%v]领取超值返利", user.Name)
			getElevenEnergyPrize(user.ServerURL, zoneToken, 1)
			getElevenEnergyPrize(user.ServerURL, zoneToken, 2)
			getElevenEnergyPrize(user.ServerURL, zoneToken, 3)
			getElevenEnergyPrize(user.ServerURL, zoneToken, 4)

			xlog.Infof("[%v]collectMineGold", user.Name)
			collectMineGold(user.ServerURL, zoneToken)

			dayGetGiftBoxAward(user.ServerURL, zoneToken)
			activateDayTaskGift(user.ServerURL, zoneToken)

			if id == "302691822" || id == "309392050" {
				followCompanion_1(user.ServerURL, zoneToken, 3)
			} else {
				followCompanion_1(user.ServerURL, zoneToken, 1)
			}
			xlog.Infof("---------------------------[%v]结束转盘---------------------------", user.Name)

			// serverURL := getServerURL()

			// zoneToken, familyDayTask := getEnterInfo(uid, name, serverURL, token, "familyDayTask")

			getFamilyTaskPrizeLogic(familyDayTask, user.ServerURL, zoneToken, user.Name)
...
func drawAll(intDrawMulti, intAmount int) {
	...
	for _, u := range users {
		...
		if goUid == "302691822" {
			...
			energy := draw(goUid, goName, goServerURL, goZoneToken, intDrawMulti, false)
			...
			for i := 0; i <= int(drawAmount); i++ {
				count := draw(goUid, goName, goServerURL, goZoneToken, intDrawMulti, false)
				if count == -1 {
					break
				}
				time.Sleep(time.Millisecond * 2100)
...
func DrawH1(s *web.Session) web.Result {
	...
	go func() {
		...
		energy := draw(uid, name, serverURL, zoneToken, drawMulti, false)
		...
		for i := 1; i <= intAmount; i++ {
			draw(uid, name, serverURL, zoneToken, drawMulti, false)
...
func PlayLuckyWheelH(w http.ResponseWriter, req *http.Request) {
	...
	for rows.Next() {
		var uid, name, token, zoneToken string
		rows.Scan(&uid, &name, &token, &zoneToken)
		u := []string{uid, name, token, zoneToken}
...
func attackMyBossLogic(id, mode string) (err error) {
	...
	if id == "small" {
		SQL = "select id,name,token from tokens where id not in (302691822,309392050)"
	}
...
func SonAttackBossH(w http.ResponseWriter, req *http.Request) {
	...
	for rows.Next() {
		var uid, token, name string
		rows.Scan(&uid, &token, &name)
		userIDs1 = append(userIDs1, uid)

	}
	for _, uid := range userIDs1 {
		user := GetUser(uid)
		token := user.Token
		name := user.Name
		serverURL := user.ServerURL
...
func ProcExchangeLast() (err error) {
...
func pullAnimalBySql(SQL string) {
	...
	for rows.Next() {
		...
		u := []string{uid, token, name, pullRows, followUids}
	...
	for _, u := range userIDs {
		...
		if followUids != "" {
			...
			go func() {
				...
				foods, _ := enterFamilyRob(user.ServerURL, user.ZoneToken)
...
func enterFamilyRob(serverURL, zoneToken string) (foods []map[string]interface{}, worker string) {
	now := fmt.Sprintf("%v", time.Now().UnixNano()/1e6)
...
func goldMineGift(serverURL, zoneToken, fuid, id string) bool {
	params := map[string]string{
		"fuid": fuid,
		"id":   id,
	}
...
func visitIsland(serverURL, zoneToken string, targetUid interface{}) (ids []string, attackType int) {
	now := fmt.Sprintf("%v", time.Now().UnixNano()/1e6)
	...
	if len(ids) > 0 {
		isFriend, ok := formData["isFriend"].(bool)
		if !ok {
			return
		}
		attackType = 2
...
func getAttackEnemyList(serverURL, zoneToken string) (userIDs, policeUserIDs, policeUids []int) {
	...
	formData := httpGetReturnJson(url)
	enemys, ok := formData["enemys"].([]interface{})
	if ok {
		for _, v := range enemys {
			enemy, ok := v.(map[string]interface{})
			if ok {
				userID, ok := enemy["uid"].(float64)
				if !ok {
					userIDS, ok := enemy["uid"].(string)
					if !ok {
						continue
					}
					userID, _ = strconv.ParseFloat(userIDS, 64)
				}
...
func attackIsland(serverURL, zoneToken string, flag, targetUid, building interface{}) (attackUid int, buildings string) {
	...
	if formData["addGold"] == nil {
		xflag := fmt.Sprintf("%v", flag)
		if xflag == "5" || xflag == "2" {
			if xflag == "5" {
				xflag = "2"
			} else {
			...
			now = fmt.Sprintf("%v", time.Now().UnixNano()/1e6)
			url = fmt.Sprintf("%v/game?cmd=attack&token=%v&type=%v&targetUid=%v&building=%v&now=%v", serverURL, zoneToken, xflag, targetUid, building, now)
			formData = httpGetReturnJson(url)
			xlog.Infof("再次攻击小岛------->")
		}
	}
...
// 赠送拼图
func giftPiece(serverURL, zoneToken, id, targetUid string) (errorID float64) {
	now := fmt.Sprintf("%v", time.Now().UnixNano()/1e6)
	url := fmt.Sprintf("%v/game?cmd=giftPiece&token=%v&id=%v&targetUid=%v&now=%v", serverURL, zoneToken, id, targetUid, now)
	formData := httpGetReturnJson(url)
	if error, ok := formData["error"]; ok {
		errorID = error.(float64)
		return
	}
	return
}
...
func draw(uid, userName, serverURL, zoneToken string, drawMulti interface{}, isTask bool) float64 {
	...
	if id == "10" {
		...
		if (uid == "302691822" || uid == "309392050") && !isTask {
				...
				if len(policeUserIDs) > 0 {
					...
					if len(buildings) > 0 {
						...
						for _, v := range enemys {
							...
							attackUid = v
							var buildings = []string{}
							buildings, attackType = visitIsland(serverURL, zoneToken, attackUid)
							if len(buildings) > 0 {
								building = buildings[0]
								xlog.Infof("[%v]【目标更换成复仇对象】:%v --> %v:%v", userName, attackUid, building, attackType)
								break
							} else {
								xlog.Infof("[%v] 复仇对象中没有可攻击的建筑", userName)
							}
						}
					}
				} else {
			...
			attackUid1, building1 := attackIsland(serverURL, zoneToken, attackType, attackUid, building)
...
func companionUp(serverURL, zoneToken, id string) bool {
	now := fmt.Sprintf("%v", time.Now().UnixNano()/1e6)
	url := fmt.Sprintf("%v/game?cmd=companionUp&token=%v&id=%v&now=%v", serverURL, zoneToken, id, now)
	formData := httpGetReturnJson(url)
	if _, ok := formData["error"]; ok {
		return false
	}
...
func companionActive(serverURL, zoneToken, id string) {
	now := fmt.Sprintf("%v", time.Now().UnixNano()/1e6)
	url := fmt.Sprintf("%v/game?cmd=companionActive&token=%v&id=%v&now=%v", serverURL, zoneToken, id, now)
	httpGetReturnJson(url)
}
...
func RunnerDraw() (err error) {
	...
	if hour == 4 {
		...
		for _, user := range users {
			user.ServerURL = getServerURL()
			var companionList interface{}
			user.ZoneToken, companionList = getEnterInfo(user.Uid, user.Name, user.ServerURL, user.Token, user.ZoneToken, "companionList")
			companionListMap, ok := companionList.(map[string]interface{})
			if !ok {
				continue
			}
			user.CompanionList = map[string]map[string]float64{}
			for k, v := range companionListMap {
				vMap, ok := v.(map[string]interface{})
				if ok {
					vMap2 := map[string]float64{}
					...
					user.CompanionList[k] = vMap2

					if vMap2["lv"] < 20 && vMap2["exp"] > vMap2["lvUpExp"] {
						for i := vMap2["lv"]; i <= 20; i++ {
							if !companionUp(user.ServerURL, user.ZoneToken, k) {
								break
							}
							xlog.Infof("[%v]升级宠物[%v]到[%v]", user.Name, k, i+1)
						}
					}
					if k == "2" && vMap2["lv"] == 20 && vMap2["activeDuration"] == 0 {
						companionActive(user.ServerURL, user.ZoneToken, k)
						followCompanion_1(user.ServerURL, user.ZoneToken, 3)
						xlog.Infof("[%v]激活宠物[%v]", user.Name, k)
					}
			...
			if goUid == "302691822" {
				...
				if iMax <= 0 {
					if gomaxDraw > goDayDraw {
						...
						draw(goUid, goName, goServerURL, goZoneToken, gomaxDraw-goDayDraw, false)
					} else {
					...
					// time.Sleep(time.Millisecond * 2100)
					for i := 0; i <= iMax; i++ {
						count := draw(goUid, goName, goServerURL, goZoneToken, 5, false)
						if count == -1 {
							break
						}
						time.Sleep(time.Millisecond * 2100)
	...
	if drawStatus != "1" {
		...
		go func() {
			...
			followCompanion_1(serverURL, zoneToken, 2)
			for i := 0; i < iMax; i++ {
				count := draw(cowBoyUid, cowBoyName, serverURL, zoneToken, 5, false)
				if count == -1 {
					break
				}
	...
	if hour == 0 || hour == 1 || hour == 3 || hour == 8 || hour == 11 || hour == 14 || hour == 17 || hour == 20 || hour == 22 {
		...
		drawLogic(hour, maxDraw, 5, false)
...
func drawLogic(hour int, maxDraw, drawMulti float64, isTask bool) (err error) {
	xlog.Infof("start draw")
	...
	for _, user := range users {
			...
			if ok {
				...
				if vMap2["lv"] < 20 && vMap2["exp"] > vMap2["lvUpExp"] {
					for i := vMap2["lv"]; i <= 20; i++ {
						...
						xlog.Infof("[%v]升级宠物[%v]到[%v]", user.Name, k, i)
					}
				}
				if k == "2" && vMap2["lv"] == 20 && vMap2["activeDuration"] == 0 {
					companionActive(user.ServerURL, user.ZoneToken, k)
		...
		if user.CompanionList["3"] != nil {
			followCompanion_1(user.ServerURL, user.ZoneToken, 3)
		} else {
			if user.CompanionList["1"] != nil {
				followCompanion_1(user.ServerURL, user.ZoneToken, 1)
			} else {
				followCompanion_1(user.ServerURL, user.ZoneToken, 4)
			}
		}
		user.ZoneToken, user.FamilyDayTask = getEnterInfo(user.Uid, user.Name, user.ServerURL, user.Token, user.ZoneToken, "familyDayTask")
		var dayDraw interface{}
	...
	for _, u := range users {
		...
		goFamilyDayTask := u.FamilyDayTask
		goDayDraw := u.DayDraw
		goCompanionList := u.CompanionList
		...
		if goUid == "302691822" {
			...
			go func() {
				...
				if goCompanionList["3"] == nil {
					if goCompanionList["4"] == nil {
						followCompanion_1(goServerURL, goZoneToken, 1)
					} else {
						followCompanion_1(goServerURL, goZoneToken, 4)
					}
				} else {
					followCompanion_1(goServerURL, goZoneToken, 3)
				}
...
// 1=护盾防御住1次攻击 2=领取1次免费20能量 3=在公会聊天发言1次 4=领取1点好友赠送能量 5=使用转盘100次 6=攻击仇人1次 7=通过转盘获得3个护盾 8=偷对土豪2次 9=自己召唤的恶龙被击杀 11=使用魔法药水重置糖果树 12=使用时光机重置小岛 13=在糖果树上打出1个糖果 14=在小岛寻宝中夹到1个宝石 15=分享3次
func getFamilyTaskPrizeLogic(goFamilyDayTask interface{}, goServerURL, goZoneToken, goName string) (result string) {
	...
	if runnerStatus("SaturdayAttackMyBossStatus") == "1" {
		if time.Now().Weekday() != time.Sunday {
			...
			if now.Minute() < 35 {
				...
				xlog.Infof("准备执行任务转盘,等待%v", s.Sub(now))
...
func InitTodayAnimal() (err error) {
	...
	for rows.Next() {
		...
		u := []string{uuid, uname, utoken, uzoneToken}
		users = append(users, u)
	}
	for _, u := range users {
		var uuid, uname, utoken, uzoneToken string
...
// TODO: 未完成
// func doneUserGoldMine(userID string) (result string) {
// 	AllLock.Lock()
// 	defer AllLock.Unlock()
// 	user := GetUser(userID)
			...
			for _, friendUserID := range friendUserIDs {
				...
				if canDonate > 0 {
					...
					for i := 0; i < int(canDonate); i++ {
						xlog.Infof("%s 开始捐给 %s 一个 拼图%s", friendUser.Name, user.Name, pieceID)
						errorID := giftPiece(friendUser.ServerURL, friendUser.ZoneToken, pieceID, userID)
						if errorID == 49 {
							xlog.Infof("%s 捐给 %s 一个 拼图%s 失败！！！", friendUser.Name, user.Name, pieceID)
							break
						}
						if errorID == 0 {
							xlog.Infof("%s 捐给 %s 一个 拼图%s 完成", friendUser.Name, user.Name, pieceID)
							need--
						} else {
							return
						}
...
func getMiningScoreReward(serverURL, zoneToken string) bool {
	...
	form := httpGetReturnJson(URL)
	xlog.Infof("getMiningScoreReward %v", converter.JSON(form))
